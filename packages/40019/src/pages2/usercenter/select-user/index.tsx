...
export default memo(() => {
  const { pageRoute, type } = useGetParams<{
    pageRoute: string;
    type?: string;
  }>();
  const {
  ...
  const onCardClick = useCallback(
    async (patient: PatientType) => {
      ...
      if (type === 'tjzx') {
        window.location.href = `http://183.65.22.8:8190/MedicalHomePage?peId=${patient?.patHisNo}`;
      }
      if (pageRoute) {
    ...
    [defaultPatientInfo.patientId, pageRoute, setDefaultPatientInfo, type],
  );
