...
import { DoctorInfo, Calendar } from 'commonHis/src/pages2/register/components';
import { Button, Space, showToast, Loading } from '@kqinfo/ui';
import useGetParams from '@/utils/useGetParams';
import Items from 'commonHis/src/pages2/register/select-time/items';
...
export default () => {
  ...
  const {
    ...
    params: {
      deptId,
      doctorId,
      extFields: { TjFlag: isHealth === '1' ? '1' : '0' },
    },
  ...
  const handleNext = useCallback(async () => {
    ...
    navigateTo({
      url: `/pages2/register/confirm/index?deptId=${deptId}&doctorId=${doctorId}&scheduleDate=${date.format(
        'YYYY-MM-DD',
      )}&scheduleId=${scheduleId}&visitBeginTime=${visitBeginTime}&visitEndTime=${visitEndTime}&visitPeriod=${visitPeriod}&sourceType=${sourceType}&level=${level}&title=${title}&doctorName=${doctorName}&TjFlag=${
        doctorScheduleDateDetail?.extFields?.TjFlag || ''
      }`,
    });
  }, [
    date,
