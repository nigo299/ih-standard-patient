...
export default () => {
  const { type = 'default', isHealth = '0' } = useGetParams<{
    type: 'reserve' | 'day' | 'default' | 'health';
    isHealth: '0' | '1';
  }>();
      ...
      {CHILDREN_DEPTLIST ? (
        ...
        deptList?.map((dept) => (
          <Space
            ...
            onTap={() =>
              navigateTo({
                url: `/pages2/register/select-doctor/index?deptId=${dept.no}&type=${type}&isHealth=${isHealth}`,
              })
            }
          >
            <View>{dept.name}</View>
