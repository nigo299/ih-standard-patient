...
import { decrypt, getCurrentPageUrl, goHealthUrl, sleep } from '@/utils';
...
import { getVaildJSON } from '@/pages2/register/order-detail';
      ...
      const { code, data, msg } = await useApi.锁号.request({
        ...
        medinsureChannel:
          PLATFORM === 'ali'
            ...
            ? 1
            : 2,
        extFields: JSON.stringify({ TjFlag }),
      });
      if (code === 0 && data.orderId) {
        const res = await useApi.查询挂号订单详情.request({
