...
export default () => {
  ...
  const getWaitOpList = useCallback(async () => {
    ...
    try {
      data.some((item: { extFields: any }) => {
        const extFields = JSON.parse(item?.extFields || '{}');
        if (extFields?.skinResult === '2') {
          setVisible(true);
          return true;
        }
        if (extFields?.skinResult === '1') {
          setVisible1(true);
          return true;
        }
    ...
    // console.log(extFields, 'extFields');
    ...
    if (code === 0 && data?.length >= 1) {
      ...
      const waitPayList = diffsArr?.map((item) => ({
        title: `${
      ...
      setWaitOpList(waitPayList);
      const WaitPayHisOrderNoArrs: string[] = []; //等待支付的订单的hisOrderNo
      //遍历取出所有
  ...
  return (
    <View className={styles.wrap}>
      ...
      >
        <Space style={{ lineHeight: 1.2, padding: 20 }}>
          <View>皮试结果为阳性，不能缴费，请联系医生处理！</View>
        </Space>
      </Dialog>
