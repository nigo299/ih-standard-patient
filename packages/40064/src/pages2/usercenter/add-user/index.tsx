...
export default memo(() => {
          ...
          const { code } = await handleAdd(
            btnSubType === 'bind'
              ...
              : {
                  ...
                  extFields: {
                    regionCode: regionCode || '50010600',
                  },
                  idNo:
  ...
  const showUndocumented = useMemo(
    () => !checked && isChildren,
    [checked, isChildren],
  );
  return (
          ...
          {!checked && bindcardProdiles?.patientTypes && (
            ...
            >
              <Picker
                ...
                onChange={(e) => {
                  if (e !== '1') {
                    if (form.getFieldValue('idType') === '100') {
                      form.setFieldValue('idType', '1');
                    }
                  }
                  setIsChildren(e === '1');
                }}
            ...
            <Picker
              ...
              onChange={(e) => {
                if (e === '100') {
                  showModal({
                    title: '温馨提示',
                    content: '仅出生日期小于45天的新生儿才可选择无证件建档',
                    showCancel: false,
                  });
                }
              }}
              data={bindcardProdiles?.idTypes
                ?.filter((item) =>
                  showUndocumented ? showUndocumented : item?.dictKey !== '100',
                )
                .map((item) => ({
                  label: item?.dictValue,
                  value: item?.dictKey,
                      ...
                      >
                        <TransferChange mode={'city'}>
                          <Picker
                            ...
                            data={addressOptions}
                            onChange={(v) => {
                              console.log(v, 'picker');
                              setRegionCode(v?.[2] + '00');
                            }}
                          >
                            请选择省市区
                          </Picker>
