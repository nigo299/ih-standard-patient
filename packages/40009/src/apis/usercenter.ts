...
export default {
  ...
  完善就诊人基础信息: createApiHooks((params: Partial<PatientInfoType>) =>
    request.post<HelathCardInfoType>(
      `/api/intelligent/personal/complete-patient`,
      params,
    ),
  ),
  ...
  查询就诊人是否需要完善基础信息: createApiHooks(
    ({ patientId }: { patientId: string }) =>
      request.get<{ code: number; data: boolean }>(
        `/api/intelligent/personal/need-complete/${patientId}`,
      ),
  ),
