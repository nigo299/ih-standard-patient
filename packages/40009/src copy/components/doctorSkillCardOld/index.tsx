...
import { DatescheduleType } from '@/apis/register';
...
export interface IProps {
  ...
  doctorSourceList: DatescheduleType[];
  /** 当前选中的日期 */
  current: string;
...
export default ({
  ...
  const deptSourceListView = useMemo(() => {
    const Index = doctorSourceList.findIndex(
      (item) => item.scheduleDate === current,
    );
    const prevArr = doctorSourceList.slice(Index);
    const nextArr = doctorSourceList.slice(0, Index);
    return [...prevArr, ...nextArr];
  }, [current, doctorSourceList]);
  return (
    <Space vertical className={styles.card}>
      <Sheet ref={sheetRef}>
        <Space vertical className={styles.sheet}>
          ...
          >
            <View className={styles.sheetTitle}>医生介绍</View>
            <Button
              block={false}
              type="primary"
              elderly
          ...
          <View className={styles.sheetText}>{doctorGood}</View>
          <PartTitle elderly full bold>
            医生简介
          </PartTitle>
          <View className={styles.sheetText}>{doctorSkill}</View>
        </Space>
      </Sheet>
      ...
      <Space className={styles.head}>
        ...
        <Space vertical justify="center">
          ...
          <View className={styles.deptName2}>{deptName}</View>
          <Exceed className={styles.deptSkill}>{`擅长: ${
            doctorSkill || '暂无'
          }`}</Exceed>
        </Space>
        <Space
        ...
        {deptSourceListView.length >= 1 &&
          deptSourceListView.slice(0, 4).map((dept, index) => (
            <Space
              ...
              onTap={() =>
                dept.status === 1 && onSelectDate(dayjs(dept.scheduleDate))
              }
            >
